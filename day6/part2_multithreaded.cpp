...
int main()
{
    ...
    while(true)
    {
        STEP_RESULT step = guardStep(startPos, movementDir, obstacles, visited_tiles);
        if (step == MAP_EDGE) break;
        results.push_back(std::async(&tryLooping, startPos, movementDir, obstacles, visited_tiles));
    }
    for (auto& r : results)
    {
        bool res = r.get();
